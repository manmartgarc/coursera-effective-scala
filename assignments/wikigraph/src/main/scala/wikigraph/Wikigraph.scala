...
final class Wikigraph(client: Wikipedia):
  ...
  /** Retrieves the names of the articles linked in a page.
    ...
    *
    * Hint: Use the methods that you implemented in WikiResult. Hint: You will
    * have to perform conversions between collection types because retrieving
    * the links from an article returns a `Set`, but the method
    * `WikiResult.traverse` requires a `Seq`. Also, the method
    * `WikiResult.traverse` returns a `Seq`, but, ultimately, you want to return
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client
      ...
      .flatMap(articleIds => {
        WikiResult
          .traverse[ArticleId, String](articleIds.toSeq)(client.nameOfArticle)
          .map(e => e.toSet)
      })
  ...
  /** Computes the distance between two pages using breadth first search.
    *
    * @param start
    *   compute the distance from this page to `target`
    * @param target
    *   compute the distance from `start` to this page
    * @param maxDepth
    *   stop if the depth exceeds this value
    *
    * @return
    *   the result of an asynchronous computation that might fail. If the
    ...
    * fallback by ignoring the problematic page. On the other hand, any system
    * failure just ends the algorithm by returning that system failure.
    *
  ...
  def breadthFirstSearch(
    ...
    /** This recursive method iterates on the graph.
      ...
      *   - When the queue is empty or the maxDepth is reached (in the next
      *     element of the queue), the search fails with None
      *   - Otherwise, the next page is retrieved from the queue, and its linked
      *     pages are fetched from the dataset. The search succeeds if `target`
      *     is in this set of linked pages. Otherwise, it continues iterating on
      *     the queue by recursively calling `iter`. The `visited` argument of
      *     the recursive call should include the currently `visited` pages and
      *     the set of linked pages that were just fetched. The `q` argument of
      *     the recursive call should be:
      *     - a queue that contains the remaining pages whose linked pages need
      *       to be fetched (the current `q` without its first element),
      *     - and, to which the set of linked pages that were just fetched, and
      *       that have ''not'' already been visited, have been enqueued (with
      *       the correct distance).
      *
      * @param visited
      *   the pages that have already been visited (that we know are not the
      *   `target` page we are looking for), and that we must not add to the
      *   queue again to not iterate infinitely on graph cycles.
      * @param q
      *   the next pages whose linked pages need to be fetched, paired with
      *   their distance from the `start` page.
      *
      * HINT: Have a look at the implementation of [[wikigraph.WikiResult#zip]]
      * to see how to use [[wikigraph.WikiResult#flatMap]] to work with the
      * content of [[wikigraph.WikiResult]]. This is useful to chain successive
      * calls to iter.
      *
  ...
  /** Computes the distances between the articles whose list of titles is
    * provided.
    *
    ...
    * @param maxDepth
    *   stop the search when this value of distance is exceeded
    *
    * @return
    *   An asynchronous result containing a list of tuples with the following
    *   elements:
  ...
  def distanceMatrix(
    ...
    val pairs =
      for {
        ...
        targetArticle <- titles
        if sourceArticle != targetArticle
      } yield (sourceArticle, targetArticle)

    WikiResult.traverse[(String, String), (String, String, Option[Int])](
      pairs
    ) { (sourceArticle, distTitle) =>
      client.searchId(sourceArticle).zip(client.searchId(distTitle)).flatMap {
        (sourceId, distId) =>
          breadthFirstSearch(sourceId, distId, maxDepth).map { dist =>
            (sourceArticle, distTitle, dist)
          }
      }
    }
end Wikigraph
