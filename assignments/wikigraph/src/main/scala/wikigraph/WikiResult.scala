...
/** The result of an asynchronous computation which may fail. This class is
  ...
  * @param value
  *   the future to use to create this WikiResult. Checkout the companion object
  *   to find other ways to create a WikiResult
  *
...
case class WikiResult[A](value: Future[Either[Seq[WikiError], A]]):
  ...
  /** Recovers from domain errors by returning the provided successful
    * `solution` value instead. If this WikiResult is a system failure, that
  ...
  /** Tries to recover from domain errors by using `that` WikiResult instead. If
    * both `this` and `that` are domain errors, only the errors of `that` are
    * returned. If this WikiResult is a system failure, that failure is not
    * recovered.
    *
    * @param b
    *   the computation to use in case of failure
    */
  def fallbackTo(that: => WikiResult[A])(using
      ExecutionContext
  ): WikiResult[A] =
  ...
  def map[B](f: A => B)(using ExecutionContext): WikiResult[B] =
    val futureB: Future[Either[Seq[WikiError], B]] = value.map {
  ...
  /** Retrieve the results of two computations and produce a result containing
    ...
    *
    * Hint: The async part has been handled for you. You need to zip the two
    * Either
    */
  def zip[B](that: WikiResult[B])(using ExecutionContext): WikiResult[(A, B)] =
    def zipEithersAcc(
      ...
      (a, b) match {
        ...
        case (_, Left(errorsB))             => Left(errorsB)
...
object WikiResult:
  /** Creates a WikiResult which succeeds with the provided value
    ...
    */
  def successful[A](a: A): WikiResult[A] = WikiResult(
    Future.successful(Right[Seq[WikiError], A](a))
  )
  ...
  /** Creates a WikiResult which fails with the provided domain error
    ...
    */
  def domainError[A](e: WikiError): WikiResult[A] = WikiResult(
    Future.successful(Left[Seq[WikiError], A](Seq(e)))
  )
  ...
  /** Creates a WikiResult which fails with the provided system exception
    ...
    */
  def systemFailure[A](t: WikiException): WikiResult[A] = WikiResult(
    Future.failed(t)
  )

  /** Starts running the provided block asynchronously in the contextual
    * execution context
    *
    * @param block
  ...
  /** Asynchronously and non-blockingly transforms a `Seq[A]` into a
    ...
    *   - If the function `f` eventually returns some domain errors, returns a
    *     failed result containing all the domain errors.
    *   - If the function `f` eventually returns some system failure, returns a
    *     failed result containing one of the system failures.
    *
    * Hint: iterate over the input sequence with `foldLeft` or `foldRight`,
    * combine all the `WikiResult` values with the operation `zip` and `map`. If
    * the input sequence is empty, return a successful empty sequence. Hint:
    * this method is very similar to the method `validateEach` shown in the
    * lecture “Manipulating Validated Values”.
    */
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using
      ExecutionContext
  ): WikiResult[Seq[B]] =
    as.foldRight(WikiResult.successful(Seq.empty[B])) { (a, acc) =>
