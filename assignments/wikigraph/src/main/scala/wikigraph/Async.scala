...
object Async:
  ...
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] =
    eventuallyX.recoverWith { case NonFatal(_) => Future.successful(-1) }
  ...
  def sequenceComputations[A, B](
    ...
    asyncComputation1().flatMap { a =>
      asyncComputation2().map(b => (a, b))
    }
  ...
  def insist[A](
    ...
    asyncComputation().recoverWith { case NonFatal(_) =>
      if maxAttempts > 1 then insist(asyncComputation, maxAttempts - 1)
      else Future.failed(new Exception("Max attempts reached"))
    }
