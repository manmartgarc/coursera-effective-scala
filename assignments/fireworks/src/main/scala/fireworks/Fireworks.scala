...
object Firework:
  ...
  /** @return
    ...
    *
    * Note: in case you are familiar with object-oriented programming, you may
    * be wondering why you have to manually dispatch the call to the `next`
    * method of each subclass of `Firework`. That’s a good point. We have not
    * introduced object-oriented programming concepts yet (abstract methods and
    * inheritance), but in real-world code, the method `next` would indeed be
    * typically declared as an abstract method on the type `Firework`, and
    * implemented on every concrete subclass of `Firework`. In the coming weeks,
    * you will see how to achieve that, but for now we stick with this unusual
    * style.
    */
